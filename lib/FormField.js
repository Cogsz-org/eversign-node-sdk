...
function FormField(newFormField) {
  ...
  // Automatically set type based on constructor name if not explicitly provided
  // This ensures type is set in the internal formField object, not just this.type
  // Must be called after generateType() is defined
  if (!formField.type || formField.type === "") {
    this.generateType();
